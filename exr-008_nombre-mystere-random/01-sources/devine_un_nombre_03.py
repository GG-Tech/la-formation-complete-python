...
print("Bienvenue !")
print("Le but du jeu est de trouver le nombre aléatoire entre 0 et 10")
...
while (True):
    ...
    if (nombre_utilisateur > nombre_mystere):
        print(f"Trop grand ! Essayez plus petit que {str(nombre_utilisateur)}.")
    if (nombre_utilisateur < nombre_mystere):
        print(f"Trop petit ! Essayer plus grand que {str(nombre_utilisateur)}.")
    if (nombre_utilisateur == nombre_mystere):
        end_game(tries)
        break
